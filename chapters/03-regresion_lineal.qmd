...
1.  **Linealidad:** La relación entre $\beta$ y $Y$ debe ser lineal.
    -   **¿Para qué sirve?** Si la relación no es lineal, nuestro modelo de línea recta será intrínsecamente incorrecto. Podemos verificar esto visualmente con un diagrama de dispersión.
...
Ahora es su turno. En grupos, vamos a derivar las soluciones para la Regresión Lineal Múltiple.

### Tarea 1: Derivar las Ecuaciones Normales para el caso Múltiple

Usando la función de costo matricial $J(\boldsymbol{\beta})$, encuentren el vector de coeficientes $\boldsymbol{\beta}$ que la minimiza.
...
$$-2\mathbf{X}^T(\mathbf{y} - \mathbf{X}\hat{\boldsymbol{\beta}}) = 0$$
$$\mathbf{X}^T\mathbf{y} - \mathbf{X}^T\mathbf{X}\hat{\boldsymbol{\beta}} = 0$$
$$\mathbf{X}^T\mathbf{X}\hat{\boldsymbol{\beta}} = \mathbf{X}^T\mathbf{y}$$
$$\hat{\boldsymbol{\beta}} = (\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{y}$$
  
Esta es la famosa solución de Mínimos Cuadrados Ordinarios (MCO) en forma matricial.
:::

### Tarea 2: Formular el Descenso en Gradiente para el caso Múltiple

Basándose en el gradiente que se les dio en la pista anterior y el algoritmo que vimos para el caso simple, escriban la regla de actualización para el **vector** $\boldsymbol{\beta}$ en el Descenso en Gradiente.

**Preguntas a responder:**
1.  ¿Cuál es el gradiente de la función de costo?
2.  ¿Cómo se ve la regla de actualización para el vector $\boldsymbol{\beta}$ completo?
...
2. La regla de actualización vectorial es:
  $$\boldsymbol{\beta} := \boldsymbol{\beta} - \alpha \nabla J(\boldsymbol{\beta})$$
  $$\boldsymbol{\beta} := \boldsymbol{\beta} - \alpha (-2\mathbf{X}^T(\mathbf{y} - \mathbf{X}\boldsymbol{\beta}))$$
  Simplificando (el 2 a menudo se absorbe en la tasa de aprendizaje $\alpha$):
  $$\boldsymbol{\beta} := \boldsymbol{\beta} + \alpha \mathbf{X}^T(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})$$
:::
